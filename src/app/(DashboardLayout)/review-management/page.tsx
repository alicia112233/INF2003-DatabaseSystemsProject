...
export default function ReviewManagement() {
  ...
  const [gameIdSuggestions, setGameIdSuggestions] = useState<string[]>([]);
  ...
  const fetchReviews = async () => {
    try {
      ...
      generateSuggestions(reviewsData);
  ...
  // Generate suggestions for autocomplete
  const generateSuggestions = (reviews: Review[]) => {
    // Get unique game IDs and sort them
    const uniqueGameIds = [...new Set(reviews.map(review => review.gameId.toString()))]
      .sort((a, b) => parseInt(a) - parseInt(b));
    
    // Get unique user IDs and sort them
    const uniqueUserIds = [...new Set(reviews.map(review => review.userId.toString()))]
      .sort((a, b) => parseInt(a) - parseInt(b));
    
    setGameIdSuggestions(uniqueGameIds);
    setUserIdSuggestions(uniqueUserIds);
  };

  // Filter suggestions based on input
  const getFilteredGameIds = (inputValue: string) => {
    if (!inputValue) return gameIdSuggestions.slice(0, 10); // Show first 10 if no input
    return gameIdSuggestions
      .filter(id => id.startsWith(inputValue))
      .slice(0, 10); // Limit to 10 suggestions
  };

  const getFilteredUserIds = (inputValue: string) => {
    if (!inputValue) return userIdSuggestions.slice(0, 10); // Show first 10 if no input
      ...
      <Box>
            ...
            <Card variant="outlined" sx={{ mb: 3, bgcolor: 'grey.50' }}>
              <CardContent>
                ...
                <Grid container spacing={2} alignItems="center" mb={3}>
                  ...
                  {/* Enhanced Search Field with smart suggestions */}
                  {filterType === 'all' && (
                    <Grid size={{ xs: 12, sm: 6, md: 4 }}>
                      <Autocomplete
                        ...
                        options={(() => {
                          ...
                          if (isNumeric) {
                            // Show both game IDs and user IDs that start with the number
                            const gameIds = getFilteredGameIds(searchTerm).map(id => `Game ID: ${id}`);
                            const userIds = getFilteredUserIds(searchTerm).map(id => `User ID: ${id}`);
                            return [...gameIds, ...userIds].slice(0, 8); // Limit to 8 total
                          }
                          
                          return []; // No suggestions for text searches
                        })()}
                        value={searchTerm}
                        onInputChange={(_, newValue) => {
                          // Extract just the number if a suggestion was selected
                          if (newValue?.includes(':')) {
                            const match = newValue.match(/(\d+)/);
                            setSearchTerm(match ? match[1] : newValue);
                          } else {
                        ...
                        renderOption={(props, option) => (
                          <Box component="li" {...props}>
                            {option.includes('Game ID') ? (
                              <IconDeviceGamepad size={16} style={{ marginRight: 8 }} />
                            ) : (
                  ...
                  {/* Game ID Filter with Autocomplete */}
                  {filterType === 'gameId' && (
                    <Grid size={{ xs: 12, sm: 6, md: 4 }}>
                      <Autocomplete
                        freeSolo
                        options={getFilteredGameIds(gameIdFilter)}
                        value={gameIdFilter}
                        onInputChange={(_, newValue) => {
                          setGameIdFilter(newValue);
                        }}
                        renderInput={(params) => (
                          <TextField
                            {...params}
                            ...
                            InputProps={{
                              ...params.InputProps,
                              startAdornment: <IconDeviceGamepad size={18} style={{ marginRight: 8 }} />
                            }}
                          />
                        )}
                        renderOption={(props, option) => (
                  ...
                  {/* User ID Filter with Autocomplete */}
                  {filterType === 'userId' && (
                    <Grid size={{ xs: 12, sm: 6, md: 4 }}>
                      <Autocomplete
                        freeSolo
                        options={getFilteredUserIds(userIdFilter)}
                        value={userIdFilter}
                        onInputChange={(_, newValue) => {
                        ...
                        renderInput={(params) => (
                          <TextField
                            ...
                            InputProps={{
                              ...params.InputProps,
                              startAdornment: <IconUser size={18} style={{ marginRight: 8 }} />
                            }}
                          />
                        )}
                        renderOption={(props, option) => (
        ...
        >
          ...
          <DialogContent>
            <Box sx={{ pt: 2 }}>
              <Typography variant="body2" color="text.secondary" mb={2}>
                Editing review by User {selectedReview?.userId} for Game {selectedReview?.gameId}
              </Typography>
              
              <Box mb={3}>
                <Typography component="legend" variant="body2" mb={1}>Rating</Typography>
                <Rating
                  value={editRating}

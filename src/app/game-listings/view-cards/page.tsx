...
const ViewGames = () => {
    ...
    useEffect(() => {
        fetch('/api/game-listings/view-cards')
        .then((res) => res.json())
        .then(data => setGames(data.games || []))
        .catch((err) => console.error('Error fetching card games:', err));
    }, []);

    if (games.length === 0) {
        return (
            <Box sx={{ mt: 4 }}>
                <Typography variant="h6" color="text.secondary">No card games found.</Typography>
            </Box>
        );
    }
    ...
    return (
    <Box sx={{ mt: 3 }}>
        <Typography variant="h6" sx={{ mb: 6 }} color="text.secondary">Shuffle up and deal! Whether it’s trading cards or party games, these hits never get old.</Typography>
    ...
    return (
        <Layout>
            <PageContainer title="Game Haven" description="Your ultimate gaming destination">
                <Typography variant="h4" fontWeight={600} mb={1}>Card Games</Typography>
                    <ViewGames />
            </PageContainer>
        </Layout>
  );

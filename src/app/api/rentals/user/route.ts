...
export async function POST(request: NextRequest) {
  try {
    ...
    // Temporarily disable stock check - allow all rentals for now
    // TODO: Re-enable stock checking once database is properly seeded
    /*
    // Check if game is available for rental (has stock)
    // If stock_count is null or undefined, assume it's available (fallback for data issues)
    const stockCount = game.stock_count || 0;
    if (stockCount <= 0 && game.stock_count !== null) {
      await connection.end();
      return NextResponse.json({ 
        error: 'Game not available for rental', 
        details: `Stock count: ${stockCount}` 
      }, { status: 400 });
    }
    */
